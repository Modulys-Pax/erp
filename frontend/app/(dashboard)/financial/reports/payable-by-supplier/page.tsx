...
import { ACCOUNT_PAYABLE_STATUS_LABELS } from '@/lib/constants/status.constants';
...
export default function PayableBySupplierReportPage() {
  ...
  const { data: report, isLoading } = useQuery({
    queryKey: [
      'report-payable-by-supplier',
      effectiveBranchId ?? undefined,
      startDate || undefined,
      endDate || undefined,
    ],
    queryFn: () =>
      accountPayableApi.getReportBySupplier(
        effectiveBranchId ?? undefined,
        startDate || undefined,
        endDate || undefined,
      ),
  ...
  const exportData = useMemo((): ExportRow[] => {
    ...
    return report.groups.flatMap((g) =>
      g.items.map((item) => ({
        Fornecedor: g.supplierName,
        Descrição: item.description,
        Valor: item.amount,
        Vencimento: formatDate(item.dueDate),
        Status: ACCOUNT_PAYABLE_STATUS_LABELS[item.status as keyof typeof ACCOUNT_PAYABLE_STATUS_LABELS] ?? item.status,
      })),
    );
  }, [report]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Relatório agrupado por fornecedor (período e filial)"
        actions={
          exportData.length > 0 ? (
            <ExportButton<ExportRow>
              ...
              filename="contas-a-pagar-por-fornecedor"
              title="Contas a Pagar por Fornecedor"
            />
          ) : null
        }
      ...
      <SectionCard title="Filtros">
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {report.groups.map((g) => (
                    <tr key={g.supplierId ?? 'sem-fornecedor'} className="border-b">
                      <td className="py-2 font-medium">{g.supplierName}</td>
            ...
            <div className="mt-6 space-y-6">
              {report.groups.map((g) => (
                <div key={g.supplierId ?? 'sem-fornecedor'}>
                  <h3 className="font-medium text-foreground mb-2">
                    {g.supplierName} — {formatCurrency(g.total)} ({g.count} conta(s))
                  </h3>
                      ...
                      {g.items.map((item) => (
                        <tr key={item.id} className="border-t">
                          ...
                          <td className="py-2 px-2">
                            {ACCOUNT_PAYABLE_STATUS_LABELS[item.status as keyof typeof ACCOUNT_PAYABLE_STATUS_LABELS] ?? item.status}
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          <p className="text-sm text-muted-foreground py-8 text-center">
            Nenhuma conta a pagar no período. Ajuste os filtros ou o período.
          </p>
        )}
      </SectionCard>
    </div>
